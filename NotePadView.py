...
class Notepad:
    ...
    def check_db_status(self):
        ...
        if self.db_status:
            ...
            messagebox.showerror("DataBase Error","cannot Connect To The DataBase")
            print(traceback.format_exc())

    def exit_func(self, event=None):
        try:
            if self.text_changed:
                mbox=messagebox.askyesno("File Note Saved!","Do You Want To Save The File?")
                if mbox is True:
    ...
    def say_something(self):
        try:
            ...
            if self.takeAudio=="":
                ...
                self.new_file()
            elif self.takeAudio.lower()=='file save':
                self.save_as()
            elif self.takeAudio.lower()=='exit':
                self.exit_func()
            elif self.takeAudio.lower()=='text bold':
                self.change_bold()
            elif self.takeAudio=='text Italic':
                self.change_italic()
            elif self.takeAudio.lower()=='search':
                self.find_func()
            elif self.takeAudio.lower()==' hide status bar':
                self.hide_statusbar()
    ...
    def change_theme(self):
        ...
        self.fg_color,bg_color=self.color_tuple
        self.text_editor.config(background=bg_color,foreground=self.fg_color)
    ...
    def secure_file(self, event=None):
        if self.db_status:
            ...
            #frame
            self.file_frame=ttk.LabelFrame(self.file_dialouge,text='Add Secure File')
        ...
        #scrolllabel list
            ...
            #self.fileList_place(relx=0.0,rely=0.30,relheight=1,relwidth=0.972)
    ...
    def add_file(self):
        ...
        try:
            if self.file_owner_input.get()and self.file_pwd_input.get()!= "":
                ...
                self.file_path = filedialog.askopenfilename(title="select files", filetypes=[("text document", "*.*")])
